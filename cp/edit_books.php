              ...
              <tbody>
                  ...
                  <?php
                                ...
                                while($insert=mysqli_fetch_array($query)){
                                    ...
                                    $title=$insert['name'];
                                    $des=$insert['des'];
                                    $gelink=$insert['gelink'];
                                    $enlink=$insert['enlink'];
                                    $eslink=$insert['eslink'];
                                    $rulink=$insert['rulink'];
                                    $frlink=$insert['frlink'];
                                    $itlink=$insert['itlink'];
                                    $img=$insert['img'];
                                    $time=$insert['time'];
    ?>
                <tr>
                    ...
                    <td>
                        <?php if (!empty($gelink)) { echo '<a target="_blank" href="' . $gelink . '">GE</a>'; }?>
                        <?php if (!empty($enlink)) { echo '<a target="_blank" href="' . $enlink . '">EN</a>'; }?>
                        <?php if (!empty($eslink)) { echo '<a target="_blank" href="' . $eslink . '">ES</a>'; }?>
                        <?php if (!empty($rulink)) { echo '<a target="_blank" href="' . $rulink . '">RU</a>'; }?>
                        <?php if (!empty($frlink)) { echo '<a target="_blank" href="' . $frlink . '">FR</a>'; }?>
                        <?php if (!empty($itlink)) { echo '<a target="_blank" href="' . $itlink . '">IT</a>'; }?>
                    </td>
                    <td><?php echo $time; ?></td>
                    <td >
                        <a href="edit_book.php?edit=<?php echo "$id&tab=$ch_cat"?>" class="btn btn-warning ">
                            <i class=" icon ion-edit "></i> რედაქტირება
                        </a>
                    </td>
                </tr>
                                <?php }?>          
